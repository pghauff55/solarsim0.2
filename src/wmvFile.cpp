...
HRESULT CwmvFile::InitMovieCreation(int nFrameWidth, int nFrameHeight, int nBitsPerPixel)
{
	...
	videoInfo.dwBitRate= (nFrameWidth*nFrameHeight*bmpInfo.bmiHeader.biBitCount* m_dwFrameRate);
	...
	mt.lSampleSize			= nFrameWidth*nFrameHeight*nBitsPerPixel/8;
...
HRESULT	CwmvFile::AppendFrameUsual(int nWidth, int nHeight, LPVOID pBits,int nBitsPerPixel)
{
	...
	hr=m_pWMWriter->WriteSample(m_dwVideoInput,10000 * m_msVideoTime,WM_SF_CLEANPOINT,pSample);
