...
class CGLToMovie
{
	CwmvFile	m_MovieFile;		/*Movie File Object*/
...
public:
	...
	CGLToMovie(LPCTSTR lpszOutputMovieFileName = "Output.avi", 
		...
		DWORD dwCodec = mmioFOURCC('W','M','V','2'),	/*Video Codec for Frame Compression*/
		DWORD dwFrameRate = 30)		/*Frames Per Second (FPS)*/
		: m_MovieFile(lpszOutputMovieFileName,WMProfile_V70_2000Video, dwFrameRate)
	{
	...
	inline HRESULT RecordFrame()
	{
		...
		return m_MovieFile.AppendNewFrame( m_nWidth, m_nHeight,m_pBits,24);
	}

	inline const CwmvFile* MovieFile() const	{	return &m_MovieFile;	}
};
