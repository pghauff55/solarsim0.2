...
#define INIT_POLAR_AZ  153.0664
#define INIT_POLAR_EL -27.4970
#define INIT_DIST      20.0
...
extern SpiceDouble dt;
  /*
   * 0->9 set speed, +/- ad
   just current speed  -- in FLYING mode
   */
...
void agvHandleKeys(unsigned char key, int x, int y)
{
  ...
  if (key >= '0' && key <= '9')
	...
	case 'a':
		t_index++;
		if(t_index>30)t_index=30;
		dt+=pow(1.5,abs(t_index));
		if(dt>10000.0)dt=10000.0;
	...
	case 'z':
	  ...
	  case ',':
		  I++;
		  if(I>8)I=0;
		EyeDist=centres[I]->planet->scale*2.5;
	    break;
	  case '.':
		  I--;
		  if(I<0)I=8;
